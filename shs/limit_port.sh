...
#DNS
iptables -A OUTPUT -p udp 每sport 53 -j ACCEPT 
iptables -A INPUT  -p udp 每dport 53 -j ACCEPT
...
#蟀諉杅
iptables -A OUTPUT -p tcp 每sport 50000:60000 -m connlimit 每connlimit-above 20 -j REJECT 每reject-with tcp-reset 
iptables -A INPUT  -p tcp 每dport 50000:60000 -m connlimit 每connlimit-above 20 -j REJECT 每reject-with tcp-reset
